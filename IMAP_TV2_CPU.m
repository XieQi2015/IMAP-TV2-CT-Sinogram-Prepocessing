function  Y = IMAP_TV2_CPU(P,I0,b,Par)
...
if nargin<4
    ...
    Dy             = diff2_ND(Y,sizeS); % the 2 order TV of sinogram data
...
normD = sum(Dy.^2);
Lam   = zeros(size(Dy)); % the Lagrange multiplier in ADMM
Z     = Lam;
wb    = Z;
for k = 1:4
...
normD            = sqrt(normD);
fftnD            = zeros(sizeS); % will be used in uwhen pdating Y
for i = 1:ndim
...
for i = 1:MaxIter
    ...
    if i<=initIter
        ...
        Z = Halfreg_CPU(Dy+Lam/mu,1./(wb*mu));
    end
